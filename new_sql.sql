...
INSERT INTO
    investment.calendar (
        ...
        FROM
            ...
            LEFT JOIN (
                ...
                FROM
                    hub.holidays
            ) holidays ON calendar_date = holidays.holiday_date
    ),
    ...
    cte_lag_lead_date_2 AS (
        ...
        FROM
            cte_lag_lead_date_1 l
            LEFT JOIN calendar c ON c.`date` = l.lag_date_1
            LEFT JOIN calendar cc ON cc.`date` = l.lead_date_1
    ),
    cte_lag_lead_date_3 AS (
        ...
        FROM
            cte_lag_lead_date_2 l
            LEFT JOIN calendar c ON c.`date` = l.lag_date_2
            LEFT JOIN calendar cc ON cc.`date` = l.lead_date_2
    ),
    cte_lag_lead_date_4 AS (
        ...
        FROM
            cte_lag_lead_date_3 l
            LEFT JOIN calendar c ON c.`date` = l.lag_date_3
            LEFT JOIN calendar cc ON cc.`date` = l.lead_date_3
    )
SELECT
    `date`,
    is_business_day_br,
